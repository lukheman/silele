...
@else
    ...
    <input step="{{ $attributes->get('step')}}" max="{{ $attributes->get('max')}}" min="{{ $attributes->get('min')}}" type="{{ $type ?? 'text'}}" class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" placeholder="{{ $placeholder ?? ''}}"  wire:model={{ $attributes->get('wire:model') }} />
@endif
</div>
